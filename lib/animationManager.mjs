...
import { defaultStyle, gdStyle, infoStyle, infoStyle2, titleStyle } from "./textStyle.mjs";
...
export class AnimationManager {
  constructor(game) {
    ...
    this.readyGoContainer = new Container();
    this.game.sceneContainer.addChild(this.readyGoContainer);
  }
  ...
  // 分數增加動畫
  animateScore(newScore, duration = 2) {
    // 如果有正在進行的動畫，先停止
    this.scoreAnimations.forEach((anim) => anim.kill());
    this.scoreAnimations = [];

    const scoreDiff = newScore - this.currentScoreValue;
    if (scoreDiff === 0) return;

    // 數字遞增動畫
    // 創建一個追蹤對象
    const counter = { val: this.currentScoreValue };

    // 數字遞增動畫
    const countingAnim = gsap.to(counter, {
      val: newScore,
      duration: duration,
      ease: "none",
      roundProps: "val",
      onUpdate: () => {
        this.game.scoreText.text = `分數: ${Math.floor(counter.val)}`;
      },
      onComplete: () => {
        this.currentScoreValue = newScore;
        this.game.scoreText.text = `分數: ${newScore}`;
      },
    });
    this.scoreAnimations.push(countingAnim);
  }
  ...
  async showGameOverAnimation() {
    ...
    glow.roundRect(-170, -50, 340, 100, 50);
    glow.fill(0xffffff);
    glow.alpha = 0;
