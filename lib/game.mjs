...
class Game {
  ...
  handleCut(sprite) {
    ...
    if (isCorrect) {
      ...
      const newScore = this.score + this.currentScoreValue;
      this.score = newScore;
      this.animationManager.animateScore(newScore);
      ...
      this.comboCount = 0;
      this.currentScoreValue = 10;
      const newScore = Math.max(this.score - 15, 0);
      this.score = newScore;
      this.animationManager.animateScore(newScore);
      this.animationManager.animateCombo(this.comboCount);
      console.log("錯誤，-15分");
      ...
      this.comboCount = 0;
      this.currentScoreValue = 10;
      const newScore = Math.max(this.score - 10, 0);
      this.score = newScore;
      this.animationManager.animateScore(newScore);
      this.animationManager.animateCombo(this.comboCount);
      console.log("錯誤，-10分");
  ...
  async endGame() {
    ...
    confirmButton.on("pointerover", () => ((confirmButton.tint = 0xffda2a), confirmCon.scale.set(1.01)));
