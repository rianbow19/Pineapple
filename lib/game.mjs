import { Container, Sprite, Texture, TextStyle, Text, Graphics } from "./pixi.mjs";
...
class Game {
  ...
  async showReadyGoAnimation() {
    return new Promise((resolve) => {
      ...
      // Initially hide GO text
      goText.alpha = 0;

      // Add to scene
      this.sceneContainer.addChild(readyText);
      this.sceneContainer.addChild(goText);

      // Create animation timeline
      const timeline = gsap.timeline({
        onComplete: () => {
          this.sceneContainer.removeChild(readyText);
          this.sceneContainer.removeChild(goText);
          resolve();
        },
      });

      // Animate READY
      timeline.from(readyText, {
        alpha: 0,
        scale: 2,
        duration: 0.5,
      });
      ...
      // Hold READY
      timeline.to(readyText, {
        alpha: 1,
        duration: 0.5,
      });

      // Fade out READY and show GO
      timeline.to(readyText, {
        alpha: 0,
        scale: 0.5,
        duration: 0.3,
  ...
  async GameStart() {
    ...
    await this.showReadyGoAnimation();
    playSound(gameSound.bgm2);
  ...
  async endGame() {
    ...
    this.endText = new Text({ text: "遊戲結束", style: titleStyle });
    ...
    this.confirmText = new Text({ text: "確定", style: infoStyle2 });
  ...
  async leaderboard() {
    ...
    const scoreHeader = new Text({ text: "分數", style: infoStyle2 });
    ...
    retryButton.on(
      "pointerover",
      () => ((playSound(gameSound.select), (retryButton.tint = 0xffda2a)), retryButton.scale.set(1.05), retryText.scale.set(1.05))
    );
    retryButton.on("pointerout", () => ((retryButton.tint = 0xffffff), retryButton.scale.set(1), retryText.scale.set(1)));
