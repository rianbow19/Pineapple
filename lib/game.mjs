...
import { leaderboardData } from "./leaderboardData.mjs";
...
class Game {
  constructor() {
    ...
    this.hoverStyle = new TextStyle({
      ...
      fontWeight: "bold",
      fill: "#ffffff",
      stroke: { color: "#779938", width: 15, join: "round" },
    });
    ...
    this.questBG.stroke({ width: 10, color: 0x696969 });
    this.questBG.y = 230;
  ...
  async startTitle() {
    ...
    startButton.on("pointerover", () => {
      startText.style = this.hoverStyle;
    });
    startButton.on("pointerout", () => {
      startText.style = this.titleStyle;
    });
...
function checkLineCircleCollision(x1, y1, x2, y2, cx, cy, r) {
  ...
  // 1. 找最近點到圓心的距離
  // 2. 若距離 <= r，則表示相交
  // 3. 還要確定垂足在 [x1,y1]~[x2,y2] 範圍內 (防止線的延長線撞到)
