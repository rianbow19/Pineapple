...
class Game {
  ...
  async leaderboard() {
    ...
    // Function to create entry containers
    ...
    leaderboard.data.forEach((player, index) => {
      ...
      if (isCurrentPlayer) {
        ...
        // Animate current player's entry
        gsap
          .timeline()
          .to(scrollContainer, {
    ...
    if (this.sceneContainer.eventMode === "static") {
      this.sceneContainer.removeAllListeners();
    }
    this.sceneContainer.eventMode = "static";
    this.sceneContainer.on("wheel", throttledScroll);

    // Add encouragement message if player not on leaderboard
